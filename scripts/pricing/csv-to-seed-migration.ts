/**
 * Frontfiles — CSV to SQL seed migration converter (L5 implementation, F1.5 v3 §10; cascade D per L1 v2.2)
 *
 ...
 *
 * v3 (cascade D) drops `intrusion_level` from `pricing_format_defaults`
 * and `pricing_platform_floors` cell keys per L1 v2.2 §5.0 + F1 v4 §3.1
 * + §4.1: watermark intensity is preview-protection only, not a pricing
 * input. The validator rejects any v2-shape CSV that still carries an
 * `intrusion_level` column with a stale-template error pointing the
 * founder back to the bootstrap script (per F1.5 v3 §6.4 / §7.4).
 *
...
//  not a pricing input. Detection of stale v2-shape CSVs that still carry
//  the column happens via the stale-template check in validateFormatDefaults
//  + validatePlatformFloors below; the message references F1.5 v3 §6.4 / §7.4
//  so the founder regenerates via the bootstrap script.)
...
export function validateFormatDefaults(rows: Array<Record<string, string>>): ValidationResult {
  ...
  // v3 stale-template check (per F1.5 v3 §6.4): a parsed row carrying an
  // `intrusion_level` field indicates the CSV was generated by the v2
  // bootstrap (6 columns) and is incompatible with the v3 cell shape per
  // L1 v2.2 §5.0. Reject hard so the founder regenerates via bootstrap.
  if (rows.length > 0 && rows[0].intrusion_level !== undefined) {
    errors.push(
      `format_defaults: stale v2-shape CSV detected (carries 'intrusion_level' column). ` +
        `L1 v2.2 §5.0 drops intrusion_level from pricing dimensions; v3 schema has 5 columns ` +
        `(format,licence_class,currency,baseline_cents,calibration_basis). ` +
        `Delete the file and re-run scripts/pricing/bootstrap-calibration-csvs.ts to regenerate. ` +
        `See F1.5 v3 §6.4.`,
    )
    return { errors, warnings }
  }

  const expectedRows = 7 * 3 // 21 (was 63 in v2 — intrusion_level dropped per L1 v2.2 §5.0)
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const key = `${r.format}|${r.licence_class}`
    if (seen.has(key)) errors.push(`${where}: duplicate (format, licence_class)`)
    seen.add(key)
  }
  return { errors, warnings }
}
...
export function validatePlatformFloors(rows: Array<Record<string, string>>): ValidationResult {
  ...
  // v3 stale-template check (per F1.5 v3 §7.4): same rationale as
  // validateFormatDefaults — reject any CSV carrying an `intrusion_level`
  // column so the founder regenerates via bootstrap.
  if (rows.length > 0 && rows[0].intrusion_level !== undefined) {
    errors.push(
      `platform_floors: stale v2-shape CSV detected (carries 'intrusion_level' column). ` +
        `L1 v2.2 §5.0 drops intrusion_level from pricing dimensions; v3 schema has 4 columns ` +
  ...
  const expectedRows = 7 * 3 // 21 (was 63 in v2)
  if (rows.length !== expectedRows)
...
export function validateFloorsBelowDefaults(
  ...
  for (const d of defaults)
    defaultsByKey.set(`${d.format}|${d.licence_class}`, d)

  for (const f of floors) {
    const key = `${f.format}|${f.licence_class}`
...
export function generateSeedSql(input: SeedSqlInput): string {
  ...
  const lines: string[] = []
  lines.push('-- F2 seed (v1, EUR) — 5 pricing tables; cascade D per L1 v2.2 §5.0')
  ...
  // Block 1 — pricing_format_defaults (21 rows; v3: intrusion_level dropped per L1 v2.2 §5.0)
  lines.push(`-- Block 1: pricing_format_defaults (${defaults.length} rows)`)
  lines.push('INSERT INTO pricing_format_defaults')
  lines.push(
    '  (format, licence_class, currency, baseline_cents, table_version, effective_from, calibration_basis, calibrated_at, calibrated_by)',
  )
  lines.push('VALUES')
  lines.push(
    defaults
      .map(
        (r) =>
          `  (${quoteId(r.format)}, ${quoteId(r.licence_class)}, ${quoteId(r.currency)}, ${r.baseline_cents}, ${tableVersion}, now(), ${quoteString(r.calibration_basis)}, now(), ${quoteId(calibratedBy)})`,
      )
      .join(',\n') + ';',
  )
  lines.push('')

  // Block 2 — pricing_platform_floors (21 rows; v3: intrusion_level dropped per L1 v2.2 §5.0)
  lines.push(`-- Block 2: pricing_platform_floors (${floors.length} rows)`)
  lines.push('INSERT INTO pricing_platform_floors')
  lines.push(
    '  (format, licence_class, currency, min_cents, effective_from, calibrated_at, calibrated_by)',
  )
  lines.push('VALUES')
  lines.push(
    floors
      .map(
        (r) =>
          `  (${quoteId(r.format)}, ${quoteId(r.licence_class)}, ${quoteId(r.currency)}, ${r.min_cents}, now(), now(), ${quoteId(calibratedBy)})`,
      )
      .join(',\n') + ';',
  )
  lines.push('')
