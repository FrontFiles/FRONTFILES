/**
 * POST /api/v2/asset/[id]/proposal/override — E6 §7.3
 *
 * Body: { field: ProposalField; value: unknown; surface: AuditSurface; override_reason?: string }
 */

import { NextRequest, NextResponse } from 'next/server'
import { isRealUploadEnabled } from '@/lib/flags'
import { overrideProposal, type ProposalField } from '@/lib/ai-suggestions/proposal-mutations'
...
export async function POST(
  ...
  let body: { field?: string; value?: unknown; surface?: string; override_reason?: string }
  try {
    body = await req.json()
  } catch {
    return NextResponse.json({ code: 'bad_request' }, { status: 400 })
  ...
  await overrideProposal({
    assetId,
    creatorId,
    field: body.field as ProposalField,
    value: body.value,
