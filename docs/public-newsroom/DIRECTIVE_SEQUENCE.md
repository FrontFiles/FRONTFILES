...
| Pack-transition integration-suite stabilization | NR-D9a VERIFY 3 deferral (2026-04-25) | First run of `pack-transition.test.ts` (FF_INTEGRATION_TESTS=1) hung > 6 min before kill; subsequent isolated single-case run failed fast (1.6s) with `"Could not find the table 'public.newsroom_signing_keys' in the schema cache"`. Direct `curl` to PostgREST returned `PGRST301` — same root cause as the existing `.env.local key drift` v1.1 item. After that drift is resolved, re-run the full 20-case suite to validate: (a) the hang was a downstream symptom of the broken JWT (likely supabase-js retry loop on auth-rejected requests under Promise.all), and (b) per-test fixture cleanup ordering holds under parallel-run isolation. If the hang reproduces post-key-rotation, root-cause via vitest `--reporter verbose` and serialize problematic suites with `vi.sequential` or `concurrency: 1` in vitest config. SQL function correctness is established for v1 via direct psql RPC probe + CHECK constraint smoke + structural verifies. |
...
| Notification delivery hardening — claim-then-send loss recovery | NR-D9c §3.4 divergence (2026-04-25) | NR-D9c's `processPendingNotifications` claims (UPDATE `notification_sent_at = now()`) BEFORE sending the email, preventing duplicates across concurrent cron workers but creating a silent loss path: if Resend fails after the claim, the recipient never gets notified and the `WHERE notification_sent_at IS NULL` clause prevents retry. Acceptable for closed beta (vetted distributors, low Resend failure rate). Pre-NR-G5 hardening options: (a) separate `attempted_at` vs `sent_at` tracking columns with bounded retry on attempted-but-not-sent rows; (b) dead-letter queue for failed sends + ops dashboard; (c) revert to send-then-mark with idempotency-key dedup at Resend (Resend supports `Idempotency-Key` header). Decision deferred to launch hardening with closed-beta failure-rate data. |
...
| 2026-04-25 | NR-D9a cleared (approve with VERIFY 3 deferral) | Pack state-machine RPC. 4 new files (~1100 lines): `newsroom_pack_transition` SECURITY DEFINER function (PG plpgsql, SET search_path = public, pg_temp), symmetric DOWN rollback, `src/lib/newsroom/pack-transition.ts` server-only TS wrapper with discriminated-union return, 20-case integration test suite gated by FF_INTEGRATION_TESTS=1. Audit-first phase produced 1 IP, ratified pre-composition: rename F3/F4 from `state-machine.ts` → `pack-transition.ts` to avoid collision with NR-D4's pure UI validator (preserves client/server-only boundary). Mid-compose refinement (LEFT JOIN form for orphan-asset detection in scan-precondition check) was strictly more defensive than directive's INNER JOIN form — defends against NR-D7a two-INSERT atomicity caveat. **VERIFY 3 (full integration suite) deferred** — first run hung > 6 min, single-case isolation failed fast on `.env.local` legacy-JWT drift (existing v1.1 backlog from NR-D7b). SQL correctness established by direct psql RPC probe (`{"ok": false, "error_code": "pack-not-found"}`) + 4/4 CHECK-constraint smoke tests fire on direct UPDATE bypass + rollback smoke clean. Structural signals: typecheck ✓, build ✓ (route count unchanged at 114), full unit suite ✓ (259/259 + 20 skipped as designed), `\df newsroom_pack_transition` ✓. New v1.1 backlog item: "Pack-transition integration-suite stabilization". Two sources of truth on transition matrix (state-machine.ts validator + RPC) inherited from NR-D4 — codegen target tracked but not opened separately. **Phase NR-2 progress: 7 of 8 directives done (87.5%). NR-D9b (publish UI flow) is the last NR-2 directive.** |
| 2026-04-26 | NR-D9b cleared (approve) | Publish flow UI (P9 + P10). 9 deliverables (1 EDIT + 8 NEW, ~2200 lines): rights-warranty modal (P9) + pre-publish checklist sidebar (P10 — 7-row PRD-verbatim derivation) + publish/schedule confirmation modal (P10 3-variant body) + 2 API routes (warranty + transition) + checklist lib with zod schemas + 31-case unit test suite. F1 EDIT converts `pack-editor-shell.tsx` to async server component; preserves the 5-prop signature unchanged across all 4 callers (audit (e) confirmed compat). Audit-first phase produced 4 IPs, all ratified Option A: (IP-1) modal primitive native `<dialog>` element with `showModal()` + native focus trap + ESC + ::backdrop (no new dep); (IP-2) toast primitive native `<output role="status" aria-live="polite">` with 6s auto-dismiss + manual close (no new dep); (IP-3) schedule confirmation modal title interpolated as "Schedule this pack?" for both schedule variants — PRD silent on title, interpolated for accessibility (aria-labelledby); (IP-4) directive cited stale `canonicalPackUrl`; real export is `packCanonicalUrl(orgSlug, packSlug)` from `canonical-url.ts:17`. Carry-forwards: F8 LEFT JOIN orphan-asset semantics mirror NR-D9a's RPC; F8 conflates `error` scan results into `{n} flagged` (PRD has no error string); F7 omits application-layer status guard (RPC is the sole gate); F6 retains draft guard + sets `confirmed_by_user_id = ctx.authUserId` mirroring RLS intent though service-role bypasses; PRD §5.1 P10 7 verbatim labels frozen via `Object.freeze` with regression-guard test. **VERIFY 7 (visual smoke) deferred** — same `.env.local` JWT drift v1.1 backlog as NR-D7b/D9a; structural signals load-bearing (typecheck ✓, 290 tests ✓, build ✓ with route count 114 → 116, both new routes 401 fail-closed on curl smoke). Two new v1.1 backlog items: modal primitive promotion (to shadcn Dialog post ~5 modals), toast primitive promotion (to sonner or equivalent when multi-use). **Phase NR-2 closes. NR-G2 gate: a verified company can create, embargo, and publish a Pack in dev with signed receipts — receipt signing ships in NR-D10 (KMS + signing keys), so NR-G2 closure deferred to NR-D10. NR-D9c (lift worker + subscriber notifications) opens Phase NR-3 prep.** |
| 2026-04-26 | NR-D9c cleared (approve with VERIFY 7 partial) | Embargo lift worker + subscriber notification fanout. 9 deliverables + 2 sidecars (~1300 lines net): migration adding `notification_sent_at` column + partial index, symmetric DOWN, `publish-pipeline.ts` orchestration lib (two pure functions: `processPendingLifts` + `processPendingNotifications`, injection points for client/email/transitionPack), 15-case mock-supabase test suite, Vercel Cron route at hourly schedule, publish-notification email template mirroring `newsroom-embargo-invite.ts` shape, `schema.ts` field add, `env.ts` cron secret add, `vercel.json` second cron entry, `.env.example` sidecar (F8 standing constraint), `publish-checklist.test.ts` regression-fix sidecar. Audit-first phase produced 2 IPs, both ratified: (IP-1) Option A — fanout WHERE `notify_on='new_pack'` only; v1.1 broadens to `IN ('new_pack', 'embargo_lift')` when NR-D14 J7 wires opt-in surfaces; (IP-2) Vercel Free tier 2/2 cron cap reached. Material divergence (§3.4): claim-before-send ordering in `processPendingNotifications` prevents duplicates across concurrent cron workers but is lossy on Resend failure — acceptable closed-beta posture, hardening backlogged. State-mutation discipline preserved: F3 injects NR-D9a's `transitionPack` as the sole gate; no direct `pack.status` UPDATE in this directive's deliverables. **VERIFY 7 partial** — route layer 401/200 green, supabase-js downstream queries blocked by inherited `.env.local` JWT v1.1 item; failure-isolation pattern in F3 surfaced env failure cleanly as structured `<select>` synthetic-ID error rather than 500. Three new v1.1 backlog items: embargo_lift fanout broadening, notification delivery hardening, single newsroom cron secret consolidation. Route count 116 → 117 (+1). 305 tests pass (290 prior + 15 new), 20 skipped (NR-D9a integration suite, FF_INTEGRATION_TESTS unset). **Phase NR-2 distributor build is now end-to-end functional in dev (publish + auto-lift + fanout); NR-G2 closure blocked only by signed-receipt requirement (NR-D10 — KMS + signing keys).** |
| 2026-04-26 | NR-D10 cleared (approve) | Exit report clean across all sections; signing keys + receipts + KMS substrate landed; 4 audit IPs all PRD/schema-conformant (row-shaped mintReceipt output, real column names kid/algorithm/public_key_pem/private_key_ref, .well-known/receipt-keys per RFC 8615 + PRD §3.2, canonical-payload format locked v1); `vi.stubEnv` `process.env`-direct-read pattern applied to stub adapter; 29 vitest cases including tamper-detection regression guards; .well-known route resolves natively in Next 16. **Closes Phase NR-2 distributor build pending founder bootstrap runbook + NR-G2 validation.** |
