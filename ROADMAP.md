...
**Owner:** João Nuno Martins · **Last updated:** 2026-04-20 (post ECONOMIC_FLOW_v1 revision 4 — governing spec for offers / assignments / disputes / ledger updated with red-team pass 4 findings; prior update 2026-04-18 post-KD-9 closeout)
...
|---|---|---|
| **KD-9 micro-CCP — Restore the 16 newly-visible suites to green** | **Closed** (2026-04-18, commit `4e65176`). KD-8 closure surfaced 99 real test failures across 16 suites (90.8% overall pass rate). Pattern was mostly CCP-3-era env-flag caching + CCP-4-era `MODE`-at-module-load colliding with existing test helpers. Test-only env-stubbing helper + real-path fixture reshaping landed across 3 sub-tickets (upload / onboarding / entitlement). | INTEGRATION_READINESS §Known debt KD-9 |
...
| **ECONOMIC_FLOW_v1 revision 4 landed** (2026-04-20) | Red-team pass 4 applied. Dispute event set expanded to 5 (added `dispute.evidence_submitted`). New `disputes` table in §7 with pinned `state` enum. §8.5 Stripe charge ordering / composition counter / dual-thread emit blocks. §8.6 pre-consumer exception. §8.7 post-archival portability (GDPR Art. 20). §5 dispute cool-down. §11.5 hash-chain invariants +3. §12.1 asset-withdrawal. §12.4 reason-codes enum. F8 / F10 rewritten. 4 red-team passes total (C/I/M + R1/R2/R3/R4). Founder sign-off under §15 cleared. |
