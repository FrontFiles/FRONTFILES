...
**Governing spec for offers / assignments / disputes / ledger scope:** `docs/specs/ECONOMIC_FLOW_v1.md` — revision 4 (2026-04-20).
...
| KD-11 | `parseBody` in `src/lib/api/validation.ts` strips `formErrors` from Zod output before returning the error payload. Surfaced by KD-9.1 test hardening: 3 assertions in `src/app/api/v2/batch/__tests__/route.test.ts` had to fall back to composed error-message matching instead of structured field-level assertions. Tests and clients cannot assert on field-level signals without string matching. | KD-9.1 test hardening (2026-04-18) | Low (P3) | Narrow refactor: surface `formErrors` in the error payload returned by `parseBody` so callers (tests + clients) can assert structurally on field errors. Follow up with downstream test updates to replace composed-message assertions. |
| KD-12 | `vitest.setup.ts` auto-loads `.env.local` via `@next/env`'s `loadEnvConfig(cwd())` (landed in KD-8 fix). This is the root cause that makes the `scopeEnvVars` probe necessary across 14 call sites — tests inherit a populated env whether or not they want one, so any module that caches env at load-time must be isolated per-test. | KD-9 audit §Phase 3 + KD-9.1/2/3 closeouts (2026-04-18) | Low (P3) | Audit-first investigation: evaluate flipping the policy so tests start with an empty env unless they explicitly opt in (e.g. a per-suite `loadEnvForTest()` helper). Would retire most `scopeEnvVars` adoptions. Infrastructure change with broad test impact — requires its own audit before any code change; do not bundle with other work. |
| KD-13 | KD-9.1 found 3 assertions in `src/app/api/v2/batch/__tests__/route.test.ts` tied to a pre-`parseBody` error shape (e.g. `/already exists/`-style regex). Likely more route tests under `src/app/api/**/__tests__/` carry similar stale assertion shapes that only pass today because `scopeEnvVars` masks the underlying env-cache drift. | KD-9.1 test hardening (2026-04-18) | Low (P3) | Read-only audit first: sweep `src/app/api/**/__tests__/*.test.ts` for assertion patterns that predate the current `parseBody` contract. Fixes land in a second pass once the inventory is complete. |
| KD-14 | `scopeEnvVars` (`src/lib/test/env-scope.ts`) is now load-bearing test infrastructure with 14 call sites after KD-9 closure, but is undocumented. | KD-9 closure (2026-04-18) | Low (P4) | Docs only. Add a short section to the repo README or a new `src/lib/test/README.md` covering: when to reach for it, the full-fidelity restore guarantee (`hasOwnProperty` + delete-on-undefined-original), and the "live-read over cache" pattern it pairs with. |
| KD-15 | `src/lib/env.ts:134` called `envSchema.safeParse(process.env)` at module load, passing the whole `process.env` object. On Next.js/Turbopack client bundles this defeats `NEXT_PUBLIC_*` static inlining (Next only inlines when vars are read via explicit `process.env.NEXT_PUBLIC_X` paths), so the client received `{}` and Zod threw "Required" on every required field, blocking RootLayout render in the browser. Latent since commit `90154be` (Phase 1 foundation landing). The bug survived Phase 1 closeout, CCP 2/3/4, and KD-1/7/8/9 because `bun run build` and `bun run test` do not exercise a real browser load. | First browser-load attempt of dev server post-KD-9 (2026-04-18) | High (site could not render in any browser; latent in `main` since Phase 1 foundation) | **Closed 2026-04-18.** Patch in `src/lib/env.ts`: (a) build `rawEnv` from explicit `process.env.X` static paths so Turbopack can inline `NEXT_PUBLIC_*` on the client; (b) branch schema by runtime — full `envSchema` on the server, `envSchema.pick({…NEXT_PUBLIC_*, NODE_ENV})` on the client; (c) cast `env` export to `z.infer<typeof envSchema>` so existing server call-sites keep the full-shape type (client never reads server-only fields by design — Next.js module-graph separation enforces). Verification: dev server restart → page renders at `http://localhost:3000` with header/hero/SPOTLIGHT grid, no Zod errors in logs; `bun run test` 46/46 suites, 1,080 passed, 1 skipped (KD-2) — no regression vs KD-9 baseline. **Broader lesson for Phase 7 hardening:** add a browser-level smoke test (at minimum a real headless-chrome load of `/` with console-error assertion). "Tests green + build green" is insufficient coverage for "site renders". |
| KD-16 | Seed data references asset IDs as strings (`asset-038`, `asset-af-001`, `asset-me-001`, etc.) but the real Supabase `vault_assets.id` column is `uuid`. Every landing-page thumbnail fetch (`/api/media/[id]?ctx=thumbnail`) threw `invalid input syntax for type uuid: "asset-XXX"` in `getAssetGovernance`. Thumbnails rendered as broken placeholders; page chrome + nav + text intact. Surfaced immediately after KD-15's closure when the dev server finally rendered. | KD-15 browser-verification run (2026-04-18) | Medium (visual only — broken thumbnails on landing grid and anywhere seed-ID assets are referenced; did not block site load or Phase 2/3 work) | **Closed 2026-04-18.** Minor CCP 4 amendment in `src/lib/media/asset-media-repo.ts`: added `getEffectiveMode(assetId)` alongside `getMode()`. Global MODE stays decided by `isSupabaseEnvPresent()`; the per-asset override inspects the ID against a UUID regex — non-UUID IDs fall through to the mock path even when global MODE is `real`, because they cannot exist in `vault_assets.id` by definition. Both `getAssetGovernance` and `getReadyMedia` now call `getEffectiveMode(assetId)` instead of `getMode()`. The CCP 4 test contract is preserved: mock fixtures use non-UUID IDs by convention, so their routing through the mock path is already what every test expects. Verification: `/api/media/asset-038?ctx=thumbnail` → 200 / image/jpeg / 362 KB; landing-page screenshot shows 86/86 `<img>` loaded (`naturalWidth > 0`) with real photojournalism thumbnails; `bun run test` → 46/46 suites, 1,080 passed, 1 skipped (KD-2). No regression. Longer-term (Phase 6): Waterdog beta migration will land real UUIDs; this fallback becomes inert but harmless. |

KD-3 through KD-6 are all handled in the CCP 2 migration as explicit deny-all + TODO entries, which is governance-compliant per Phase 1 constraints. They do not block the CCP 2 gate; they are tracked here so they are not lost. KD-7 is queued as an independent micro-CCP (Phase 1 close-aligned). KD-8 closed 2026-04-17. KD-9 is the expected fallout from KD-8 — it surfaces real test failures that were previously masked, not new regressions; it does not block Phase 2/3 design work. KD-11 through KD-14 are P3/P4 follow-ups surfaced by KD-9 closure (parseBody formErrors, vitest env-load policy, obsolete route-test assertions, `scopeEnvVars` docs); none block Phase 2/3 design work. KD-10 is reserved in `KD-9-audit.md §Run provenance` for a proposed-but-not-yet-opened inter-run auth.users flakiness candidate. **KD-15 closed 2026-04-18** — Phase 1 real-world render gap; env.ts split into server/client schemas, site renders, tests still 46/46 green. **KD-16 closed 2026-04-18** — seed-ID-vs-UUID thumbnail breakage; resolved via `getEffectiveMode(assetId)` per-asset override in `asset-media-repo.ts` (non-UUID IDs fall through to mock path); 86/86 thumbnails render, tests still 46/46 green.
