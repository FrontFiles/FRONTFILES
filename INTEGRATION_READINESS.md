...
| KD-8 | `bun test` does not load `.env.local`. Tests that import modules running `src/lib/env.ts` validation at module-load time fail with "Required" errors for `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`. RLS tests and `src/app/api/v2/batch/__tests__/route.test.ts` are affected. This was silently masked in prior CCP 2 reporting, which measured the RLS file in a shell where env was in-process. | KD-1 Phase 3 verification run (2026-04-17) | Medium (blocks reliable CI test signal; Supabase-dependent tests skip or error depending on module import graph) | Micro-CCP: configure `bunfig.toml` or add a `preload` script that loads `.env.local` before tests. Verify full suite: `bun test` should load env and the RLS + v2/batch tests should execute against the dev Supabase (or use a test-mode bypass). |

KD-3 through KD-6 are all handled in the CCP 2 migration as explicit deny-all + TODO entries, which is governance-compliant per Phase 1 constraints. They do not block the CCP 2 gate; they are tracked here so they are not lost. KD-7 and KD-8 are post-KD-1 findings — neither blocks Phase 1 closeout; both are queued as independent micro-CCPs.
