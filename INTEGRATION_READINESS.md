...
| KD-16 | Seed data references asset IDs as strings (`asset-038`, `asset-af-001`, `asset-me-001`, etc.) but the real Supabase `assets.id` column is `uuid`. Every landing-page thumbnail fetch (`/api/media/[id]?ctx=thumbnail`) throws `invalid input syntax for type uuid: "asset-XXX"` in `getAssetGovernance`. Thumbnails render as broken placeholders; page chrome + nav + text intact. Surfaced immediately after KD-15's closure when the dev server finally rendered. | KD-15 browser-verification run (2026-04-18) | Medium (visual only — broken thumbnails on landing grid and anywhere seed-ID assets are referenced; does not block site load or Phase 2/3 work) | Founder decision. Two paths: **(a)** migrate seed data to use real UUIDs, keeping the `asset-XXX` slug as a separate `slug` or `legacy_id` column if human-readability is still wanted; **(b)** route-level defensive short-circuit in `/api/media/[id]` that returns 404 (not 500) when `id` fails UUID parse. Sequencing note: Waterdog beta migration (Phase 6) will land real UUIDs and make this moot, so a minimal (b) patch now + full (a) inside Phase 6 is the lowest-total-cost path. Does not block Phase 2/3 design work. |

KD-3 through KD-6 are all handled in the CCP 2 migration as explicit deny-all + TODO entries, which is governance-compliant per Phase 1 constraints. They do not block the CCP 2 gate; they are tracked here so they are not lost. KD-7 is queued as an independent micro-CCP (Phase 1 close-aligned). KD-8 closed 2026-04-17. KD-9 is the expected fallout from KD-8 — it surfaces real test failures that were previously masked, not new regressions; it does not block Phase 2/3 design work. KD-11 through KD-14 are P3/P4 follow-ups surfaced by KD-9 closure (parseBody formErrors, vitest env-load policy, obsolete route-test assertions, `scopeEnvVars` docs); none block Phase 2/3 design work. KD-10 is reserved in `KD-9-audit.md §Run provenance` for a proposed-but-not-yet-opened inter-run auth.users flakiness candidate. **KD-15 closed 2026-04-18** — Phase 1 real-world render gap; env.ts split into server/client schemas, site renders, tests still 46/46 green. **KD-16 opened 2026-04-18** — seed-ID-vs-UUID thumbnail breakage, visual-only, founder decision pending.
